...
Build-/Laufumgebung für die Korpus-Pipeline (Ingest) und das OSCAL-Lookup-CLI. Reine Nix-Flake, keine
systemweiten Abhängigkeiten.

## Voraussetzungen

- **Nix mit Flakes** (`experimental-features = nix-command flakes`).
- Netzwerkzugang für den Ingest (lädt den Katalog von GitHub).
- Kein systemweites Python/jq/curl nötig — alles kommt aus dem Flake.
...
- **Korpus-Ort:** `$GS_CORPUS_DIR` (default `~/.local/share/it-grundschutz/corpus`). Bewusst außerhalb des
