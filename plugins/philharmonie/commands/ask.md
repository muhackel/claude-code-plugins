...
Lies dort `skills/orchestration/SKILL.md`, Abschnitt „Einzelauftrag“. Übernimm die Frage aus dem Nutzeraufruf. Ohne Frage starte `nix run "path:<root>" -- -C "<projekt>" ask </dev/null`. Mit Frage ein vollständiges Handover schreiben und mit `ask --handover <datei>` übergeben.
